...
// available  commands
char* keyworld[] = {_CMD_HELP, _CMD_CLEAR, _CMD_LIST,
		    _CMD_NAME, _CMD_VER,   _CMD_LISP};
// version subcommands
...
void print_help(void* pThis)
{
	...
	print(
	    pThis,
	    "\tversion {microrl | demo} - print version of microrl lib or "
	...
	print(
	    pThis,
	    "\tname [string] - print 'name' value if no 'string', set name "
	    "value to 'string' if 'string' present\n\r");
	print(
	    pThis,
	    "\tlisp - dummy command for demonstation auto-completion, while "
	    "inputed 'l+<TAB>'\n\r");
}
		...
		if (strcmp(argv[i], _CMD_HELP) == 0) {
			...
			if (++i < argc) {
				if (strcmp(argv[i], _SCMD_DEMO) == 0) {
					...
					print(
					    pThis,
					    " wrong argument, see help\n\r");
				}
			} else {
				print(
				    pThis,
				    "version needs 1 parametr, see help\n\r");
			}
		} else if (strcmp(argv[i], _CMD_CLEAR) == 0) {
			print(
			    pThis,
			    "\033[2J");    // ESC seq for clear entire screen
			print(pThis, "\033[H");    // ESC seq for move cursor at
						   // left-top corner
		} else if (strcmp(argv[i], _CMD_LIST) == 0) {
			print(
			    pThis,
			    "available command:\n");    // print all command per
							// line
...
char** complet(void* pThis, int argc, const char* const* argv)
{
	...
	if (argc == 1) {
		...
		for (int i = 0; i < _NUM_OF_CMD; i++) {
			// if token is matched (text is part of our token
			// starting from 0 char)
	    ...
	    (argc > 1)
	    && (strcmp(argv[0], _CMD_VER)
		== 0)) {    // if command needs subcommands
		// iterate through subcommand for command _CMD_VER array
		for (int i = 0; i < _NUM_OF_VER_SCMD; i++) {
			if (strstr(ver_keyworld[i], argv[argc - 1])
			    == ver_keyworld[i]) {
				compl_world[j++] = ver_keyworld[i];
			}
		}
	} else {    // if there is no token in cmdline, just print all available
		    // token
		for (; j < _NUM_OF_CMD; j++) {
