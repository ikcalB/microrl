...
#	define KEY_STX                                                        \
		2 /**< ^B Start of text, maintenance mode on HP console */
...
#	define KEY_ETB                                                        \
		23 /**< ^W End transmission block, not the same as EOT */
...
#	ifdef _USE_QUOTING
		quoted_token_t
		    quotes[_QUOTED_TOKEN_NMB];    // pointers to quoted tokens
#	endif
		int (*execute)(
		    microrl_t*         pThis,
		    int                argc,
		...
		char** (*get_completion)(
		    microrl_t*         pThis,
		    int                argc,
		    const char* const* argv);    // ptr to 'completion' callback
		void (*print)(
		    microrl_t* pThis,
		    const char*);    // ptr to 'print' callback
#	ifdef _USE_CTLR_C
...
#	endif
	...
	void microrl_init(
	    microrl_t* pThis,
	    void (*print)(microrl_t* pThis, const char*));

	// set echo mode (true/false), using for disabling echo for password
	// input echo mode will enabled after user press Enter.
	void microrl_set_echo(int);

	// set pointer to callback complition func, that called when user press
	// 'Tab' callback func description:
	//   param: argc - argument count, argv - pointer array to token string
	//   must return NULL-terminated string, contain complite variant
	//   splitted by 'Whitespace' If complite token found, it's must contain
	//   only one token to be complitted Empty string if complite not found,
	//   and multiple string if there are some token
	void microrl_set_complete_callback(
	    microrl_t* pThis,
	    char** (*get_completion)(microrl_t*, int, const char* const*));

	// pointer to callback func, that called when user press 'Enter'
	// execute func param: argc - argument count, argv - pointer array to
	// token string
	void microrl_set_execute_callback(
	    microrl_t* pThis,
	    int (*execute)(microrl_t*, int, const char* const*));

// set callback for Ctrl+C terminal signal
#	ifdef _USE_CTLR_C
	void microrl_set_sigint_callback(
	    microrl_t* pThis,
	    void (*sigintf)(microrl_t*));
#	endif
