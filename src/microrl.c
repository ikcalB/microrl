	...
	if (dir == _HIST_UP) {
		if (cnt >= pThis->cur) {
			...
			if (pThis->ring_buf[header]) {
				...
				if (pThis->ring_buf[header] + header
					...
					memcpy(
					    line, pThis->ring_buf + header + 1,
					    pThis->ring_buf[header]);
				} else {
					int part0 =
					    _RING_HISTORY_LEN - header - 1;
					memset(line, 0, _COMMAND_LINE_LEN);
					memcpy(
					    line, pThis->ring_buf + header + 1,
					    part0);
					memcpy(
					    line + part0, pThis->ring_buf,
		...
		if (pThis->cur > 0) {
			...
			if (pThis->ring_buf[header] + header
				...
				memcpy(
				    line, pThis->ring_buf + header + 1, part0);
				memcpy(
				    line + part0, pThis->ring_buf,
				    pThis->ring_buf[header] - part0);
			}
...
#else
                j = generate_move_cursor(
                    j, -(_COMMAND_LINE_LEN + _PROMPT_LEN + 2));
                j = generate_move_cursor(j, _PROMPT_LEN + pos);
#endif
	}
...
#	endif
		...
		if (pThis->escape_seq == _ESC_HOME) {
			...
			terminal_move_cursor(
			    pThis, pThis->cmdlen - pThis->cursor);
...
static int microrl_insert_text(microrl_t* pThis, char* text, int len)
{
	...
	if (pThis->cmdlen + len < _COMMAND_LINE_LEN) {
		memmove(
		    pThis->cmdline + pThis->cursor + len,
...
#	endif
	if (compl_token[0] != NULL) {
		...
		if (len) {
			microrl_insert_text(
			    pThis, compl_token[0] + strlen(tkn_arr[status - 1]),
...
void new_line_handler(microrl_t* pThis)
{
	...
	if (pThis->cmdlen > 0)
		hist_save_line(
		    &pThis->ring_hist, pThis->cmdline, pThis->cmdlen);
#endif
